...
mkdir /home/$user/web /home/$user/web/public /home/$user/.ssh
...
echo -e "user_add_sudo\t$user\t$password" >> /root/account_data.csv
echo
clear
echo
echo
...
dnf install https://repo.mysql.com/mysql80-community-release-el8-3.noarch.rpm -y
...
dnf module install php:remi-$php -y
dnf module enable postgresql:$pgsql -y
dnf install nginx  postgresql-server wget python3 snapd unzip zip mc bzip2 traceroute atop htop iftop util-linux-user -y
echo
echo
echo
...
rm -f ./install_software_web_server_with_db_postgresql_centos8.sh
echo
reboot
